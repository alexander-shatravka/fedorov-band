(function($) {
    ...
    function initFormPopup() {
        ...
        function formCloser() {
            jQuery('.contact-popup').removeClass('display');
            jQuery('.contact-popup').removeClass('opacity');
    ...
    function initSeviceChoose() {
        $('.service-names').on('click', 'li', function() {
            $(this).toggleClass('selected');
            var arrOfServices = $(this).parents("form").find('.service-names .selected').toArray().map(service => {
                return service.innerText.toLowerCase();
            });
            $(this).parents("form").find('.service').val(arrOfServices.join(', '));
        })
    }
    initSeviceChoose();
    ...
    function initAjaxForm() {
        ...
        jQuery('#btn-submit-2').on('click', function(e) {
            e.preventDefault();
            initformValidation();
            var errors = jQuery('#contact-form-2 .has-error');
            if (errors.length) {
                return false;
            }
